...
class CWS_WP_Help_Plugin {
	...
	public function init() {
        ...
        global $wp_roles;
        $wp_roles->add_cap( 'administrator', 'manage_docs' );
	...
	private function explain_slurp( $id ) {
		if ( current_user_can( 'manage_docs' ) && !current_user_can( 'edit_post', $id ) ) {
	...
	public function ajax_settings() {
		if ( current_user_can( 'manage_docs' ) && isset( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], 'cws-wp-help-settings' ) ) {
			$error = false;
			$refresh = false;
	...
	public function ajax_reorder() {
		if ( current_user_can( $this->get_cap( 'manage_docs' ) ) && isset( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], 'cws-wp-help-reorder' ) ) {
			$order = array();
